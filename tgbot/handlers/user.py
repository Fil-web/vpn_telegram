...
async def user_start(message: Message):
    is_admin = message.from_user.id == config.tg_bot.admin_id
    ...
    await message.answer(
        ...
        reply_markup=keyboard_start(is_admin=is_admin),
        disable_web_page_preview=True,
    )
...
async def check_subscription_handler(callback_query: CallbackQuery):
    await callback_query.answer()
    is_admin = callback_query.from_user.id == config.tg_bot.admin_id
