...
📅 Доступ к VPN предоставляется на фиксированный срок 30 дней. Это и есть оплаченный период пользования сервисом.

⏰ Если оплата не поступила вовремя, может быть предоставлен дополнительный 1 день пользования. Это запас на случай задержки платежа, а не постоянная отсрочка.

❌ Если оплата не поступает в течение этого времени, доступ к VPN отключается. Для дальнейшего пользования сервисом доступ нужно продлить на следующий 30-дневный период.

✅ При обычном использовании действует фиксированная стоимость без доплат. В оплаченный период входит до 50 ГБ трафика. Если объём трафика превышает этот лимит, дополнительные условия обсуждаются индивидуально с админом.
...
😱 Если сервис зафиксирует отписку после получения доступа, повторная выдача VPN может быть отклонена без возможности восстановления доступа.

👍 Если VPN вам больше не нужен, можно просто не продлевать доступ. Если планируете пользоваться дальше, ориентируйтесь на условия из этого раздела."""
...
class XUI:
    ...
    def from_env(env: Env):
        ...
        default_plan = XUI.Plan(
            traffic_gb=env.int("XUI_DEFAULT_TRAFFIC_GB", 50),
        ...
        gift_month_plan = XUI.Plan(
            traffic_gb=env.int("XUI_GIFT_30D_TRAFFIC_GB", 50),
