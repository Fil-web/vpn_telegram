...
Проект представляет собой основу для Telegram-бота, который проверяет подписку на канал, выдает пробный VPN-доступ, принимает оплату через YooKassa и затем отдает готовую VPN subscription-ссылку для Xray-клиентов.
...
* **Пробный период и платный доступ**: Можно выдать пробный период, затем продлевать доступ оплатой на фиксированный срок.  
...
* `VPN_TRIAL_DURATION_DAYS=1` и `VPN_TRIAL_TRAFFIC_GB=2` – Параметры пробного доступа.  
...
Отправьте команду `/start` боту в Telegram, подпишитесь на канал и убедитесь, что после проверки доступа бот выдает пробный период, показывает оплату и дает кнопки подключения для разных устройств.
